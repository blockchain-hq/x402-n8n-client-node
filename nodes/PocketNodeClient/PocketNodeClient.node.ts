      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            name: 'Manual Payment',
            value: 'manualPayment',
            description: 'Manually pay for a specific 402 response',
          },
          {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (mode === 'autoHandle') {
        ...
        } else if (mode === 'manualPayment') {
          // Manual payment mode
          const x402Response: X402Response = inputData as any;
          ...
          if (usdcMint) {
            try {
              ...
              // If we can't get USDC balance, we'll try anyway
            }
          }
          ...
          if (!paymentResult.success) {
            // Provide more helpful error messages (same as autoHandle mode)
            let errorMessage = `Payment failed: ${paymentResult.error}`;
            let errorReason = 'payment_failed';
            const errorLower = paymentResult.error?.toLowerCase() || '';
